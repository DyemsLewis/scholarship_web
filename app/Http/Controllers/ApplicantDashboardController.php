...
class ApplicantDashboardController extends Controller
{
    ...
    public function profileData(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'platform_active_scholarships' => $this->eligibilityService->activePlatformScholarships($user),
    ...
    public function uploadApplicantVerificationDocument(Request $request): JsonResponse
    {
        ...
        if (! is_string($path)) {
            throw ValidationException::withMessages([
                'document_file' => 'The profile proof could not be stored. Please try again.',
            ]);
        }
        ...
        try {
            ...
            ActivityLog::record(
                ...
                "{$user->name}'s {$documentLabel} could not be copied to Documents.",
                $request,
        ...
        User::query()
            ...
            ->each(fn (User $admin) => PortalNotification::create([
                ...
                'title' => str($documentLabel)->title().' uploaded',
                'message' => "{$user->name} submitted a {$documentLabel} for profile review.",
        ...
        ActivityLog::record(
            ...
            "{$user->name} uploaded a {$documentLabel} for profile review.",
            $request,
            [
        ...
        $message = $existing
            ? str($documentLabel)->title().' updated.'
    ...
    public function updateProfile(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'current_scholarship_status' => ['required', Rule::in(['none', 'receiving', 'pending', 'completed'])],
            'current_scholarship_details' => ['nullable', 'required_if:current_scholarship_status,receiving,pending', 'string', 'max:1000'],
